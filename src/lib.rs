...
//before published, SET AS PUBLIC!!!
pub struct Layer {
   pub x: usize,
   pub y: usize,
   pub context: Vec<Vec<char>>
}
impl Layer {
   pub fn build(&mut self, sym:char){
   ...
   pub fn copy(&self, begin:[usize; 2], end:[usize; 2]) -> Vec<Vec<char>>{
      let mut slice: Vec<Vec<char>> = Vec::with_capacity((begin[0] as i32 - end[0] as i32).abs() as usize);
   ...
   pub fn paste(&mut self, slice:&Vec<Vec<char>>, begin:[usize; 2]){
   ...
   pub fn cut(&mut self, begin:[usize; 2], end:[usize; 2], sym:char) -> Vec<Vec<char>>{
   ...
   // GEOMETRY
   pub fn horizontal(&mut self, mut x:[usize; 2], y:usize, sym:char){
      if x[0] > x[1] {(x[0], x[1]) = (x[1], x[0])};
      for step in x[0]..x[1] {
         self.context[step][y] = sym;
      };
   }
   pub fn vertical(&mut self, x:usize, mut y:[usize; 2], sym:char){
   ...
   /*
		...
		|_____|  |_|  |_|  \_|  |______|
   */
   pub fn line(&mut self, begin:[usize; 2], end:[usize; 2], sym:char){
   ...
   /*
      ...
      |_| \_\ |_____|  \____/     |_|
   */
   pub fn rect(&mut self, mut begin:[usize; 2], mut end:[usize; 2], sym:char){
   ...
   */
  pub fn round(&mut self, center:[i32; 2], radius:i32, char:char){
   let a = [center[0], center[1] - radius];
   let b = [center[0] + radius, center[1]];
   let c = [center[0], center[1] + radius];
   let d = [center[0] - radius, center[1]];
   let mut  mover = a;
   self.context[a[0] as usize][a[1] as usize] = char;
   self.context[b[0] as usize][b[1] as usize] = char;
   self.context[c[0] as usize][c[1] as usize] = char;
   self.context[d[0] as usize][d[1] as usize] = char;
   while mover[0] != b[0] || mover[1] != b[1] {
      ...
      self.context[mover[0] as usize][mover[1] as usize] = char;
   };
...
pub struct Field<'a>{
   pub seq: Vec<&'a mut Layer>,
   pub default_char: char,
}

impl Field<'_>{
   pub fn unite(&self, layers:&Vec<&mut Layer>) -> Vec<Vec<char>>{
      //println!("{:?}", layers);
      let mut layer_result: Vec<Vec<char>> = layers[0].copy([0, 0], [layers[0].x, layers[0].y]);
      //println!("{:?}", layer_result);
      for layer in 1..layers.len() {
         for x in 0..layers[layer].context.len() { 
            for tile in 0..layers[layer].context[x].len() {
               if layers[layer].context[x][tile] != self.default_char {
